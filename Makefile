...
screen:
	bundle exec generamba gen $(modName) surf_mvp_coordinatable_module --module_path 'RepoWatcher/Flows/$(flow)' --test_path 'RepoWatcherTests/Tests/Flows/$(flow)'

## Used to create a new alert. Example: make alert modName=<AlertName> flow=<FlowName>
alert:
	bundle exec generamba gen $(modName) surf_mvp_coordinatable_alert --module_path 'RepoWatcher/Flows/$(flow)' --test_path 'RepoWatcherTests/Tests/Flows/$(flow)'
...
synx:
	bundle exec synx --exclusion "RepoWatcher/Non-iOS Resources" RepoWatcher.xcodeproj
